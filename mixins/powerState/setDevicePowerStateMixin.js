...
const { NotFoundError, ChannelNotFoundError } = require('../../lib/errors');
...
const setDevicePowerState = {
  ...
  async setDevicePowerState(deviceId, state, channel) {
    ...
    if (
      ...
      throw new ChannelNotFoundError('Device channel does not exist');
    }
    ...
    if (switches) {
      if (typeof channel === 'number') {
        ...
        params.switches = switches.map(s => {
          s.switch =
            state !== 'toggle' ? state : s.switch === 'on' ? 'off' : 'on';
          return s;
        });
      }
    } else {
      params.switch =
        state !== 'toggle' ? state : status === 'on' ? 'off' : 'on';
    }
